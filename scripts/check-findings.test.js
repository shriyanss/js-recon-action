...
const HIGH = {
  ...
  severity: "high",
  message: "msg",
  findingLocation: "/foo.js",
};
const MEDIUM = {
  ...HIGH,
  ruleId: "r2",
  ruleName: "Test Medium",
  severity: "medium",
};
...
test("medium threshold blocks medium and high", () => {
  ...
  assert.equal(
    result.stdout.trim(),
    "2",
    "should count 2 findings (high + medium)",
  );
});
...
test("low threshold blocks low, medium, high", () => {
  ...
  assert.equal(
    result.stdout.trim(),
    "3",
    "should count 3 findings (high + medium + low)",
  );
});
...
test("info findings never counted at medium threshold", () => {
  ...
  assert.equal(
    result.status,
    0,
    "info finding should not trigger medium threshold",
  );
});
...
test("count-only mode returns correct count", () => {
  ...
  assert.equal(
    result.stdout.trim(),
    "2",
    "count-only should return 2 high findings",
  );
});
...
test("count-only returns 0 for missing file", () => {
  ...
  assert.equal(
    result.stdout.trim(),
    "0",
    "count-only should return 0 for missing file",
  );
});
