...
if (!filePath || !existsSync(filePath)) {
  if (!countOnly) {
    console.log(
      "[js-recon] No analyze.json found. Skipping vulnerability check.",
    );
  } else {
    console.log("0");
  }
...
if (matched.length === 0) {
  console.log(`[js-recon] No findings at or above severity "${threshold}".`);
  exit(0);
}
...
for (const f of matched) {
  const rule = (f.ruleName || f.ruleId || "unknown")
    .substring(0, 39)
    .padEnd(40);
